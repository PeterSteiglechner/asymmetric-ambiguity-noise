...
meta_data["identity-dependent-noise"] = (
    meta_data["identity-dependent-noise"]
...
# %%
noiseName = f"{noise}Noise"
noise_type = f"sig_{noiseSC}"
fig, axs = plt.subplots(
    ...
    figsize=(14 / 2.54, 12 / 2.54),
)
for n, (out, vmax, outName) in enumerate(
    zip(["extr_all", "std_all"], [0.4, 0.6], [r"Extremeness $E$", r"Diversity $D$"])
):
    for ax, eps in zip(axs[n, :-1][::-1], [0.3, 0.5]):
        ...
        if n == 0:
            ax.set_title(
                rf"$confidenceBound = {eps}$"
                + "\n"
                + rf"{'low' if eps==0.5 else 'high'} bias",
                fontsize=bigfs,
            )
        dftable = (
            res.loc[(res.step == T) & (res.hom == 0.0) & (res.eps == eps)]
            .pivot_table(index=noise_type, columns="sig_am", values=out, aggfunc="mean")
            .sort_index()[::-1]
        )
        ...
        sns.heatmap(
            ...
            cbar=(ax == axs[n, 0]),
            cbar_ax=axs[n, -1],
            cbar_kws={"shrink": 0.36, "label": outName},
        )
        # ax.set_aspect("equal")
        ...
        if n == 0:
            ...
            if ax.get_xlabel() == "sig_am":
                ax.set_xlabel(rf"$ambiguityNoise$", x=0.4)
            ax.text(
                1.0,
                -0.14,
                "high\nnoise",
