...
Here, we visualise the degree of polarization, diversity and extremeness in a 2x2 setting (with/without homophily and with/wihtout ambiguity).
"""
...
import io
from PIL import Image  # Load this image into PIL
...
def plot_box(df, ax, hom, eps, nu, out):
    nu_out = -1 if -1 in df["outgroupAmbiguity"].unique() else nu
    mask = (
        (df["homophily"] == hom)
        & (df["confidenceBound"] == eps)
        & (df["ingroupAmbiguity"] == nu)
...
df = pd.read_table(
    "sim_data/2025-11-02_homophilyXambiguity_sortedInitialOps.csv",
    delimiter=",",
    skiprows=6,
)
...
for out in ["extremeness", "standard-deviation", "polarization"]:
    ...
    a = plot_box(df, axs[0, 0], 0.0, 0.2, 0.0, out)
    b = plot_box(df, axs[0, 1], 0.0, 0.2, 0.1, out)
    c = plot_box(df, axs[1, 0], 0.95, 0.2, 0.0, out)
    d = plot_box(df, axs[1, 1], 0.95, 0.2, 0.1, out)
    ...
    fig.text(0.55, 1, r"$ambiguity$", fontsize=8, va="top", ha="center")
    ...
    fig.text(1, 0.5, r"$homophily$", fontsize=8, va="center", ha="right", rotation=90)
    axs[0, 1].text(
        1.2 * xmax,
        ymid,
        f"{0.0}",
    ...
    # #plt.savefig(f"figs/expHom_{out}.png", dpi=600)
    # png2 = Image.open(png1)
    # # Save as TIFF
    # png2.save(f"figs/expHom_{out}.tiff")
    # png1.close()
    plt.savefig(f"figs/expHom_{out}.pdf")
# %%
